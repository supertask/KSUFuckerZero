$(function() {
    ...
    $("#slider-range" ).slider({
        ...
        create: function() {
            handle_from.text(get_grade_name(input_from.val()));
            handle_to.text(get_grade_name(input_to.val()));
        },
        slide: function(event, ui) {
            input_from.val(ui.values[0]);
            input_to.val(ui.values[1]);
            handle_from.text(get_grade_name(ui.values[0]));
            handle_to.text(get_grade_name(ui.values[1]));
        }
    });
});
